...
update x n (m, t) = (M.adjust (\_->n) x m, t)

addTrace :: String -> State -> State
addTrace op (m, trace) = (m, trace ++ op)
...
stepComm (Let x e) s =
  do (n :!: (m,t)) <- evalExp e s
     return (Skip :!: addTrace (showLet x n) ((M.insert x n m), t))
...
stepComm (RepeatUntil c b) s = 
  do let c1 = IfThenElse b Skip (RepeatUntil c b)
     return ((Seq c c1) :!: s)
...
evalExpInt (Var x)    s = 
  do x' <- lookfor x s
     return (x' :!: s)
evalExpInt (VarInc x) s =  
  do n <- lookfor x s
     let s' = update x (n + 1) s 
     return ((n+1) :!: addTrace (showLet x (n+1)) s')
evalExpInt (VarDec x) s = 
  do n <- lookfor x s
     let s' = update x (n - 1) s 
     return ((n-1) :!: addTrace (showLet x (n-1)) s')
evalExpInt (UMinus e) s = 
  do (n :!: s') <- evalExp e s
     return (-n :!: s')
...
evalLog :: (Bool -> Bool -> Bool) -> Exp Bool ->
   Exp Bool -> State -> Either Error (Pair Bool State)
evalLog op p0 p1 s= 
  do (n0 :!: s') <- evalExpBool p0 s
     (n1 :!: s'') <- evalExpBool p1 s'
     return (op n0 n1 :!: s'')


evalComp :: (Int -> Int -> Bool) -> Exp Int ->
   Exp Int -> State -> Either Error (Pair Bool State)
evalComp op e0 e1 s = 
  do (n0 :!: s')  <- evalExpInt e0 s
