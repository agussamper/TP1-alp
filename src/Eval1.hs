...
evalExpInt (Const n)  s = evalExp (Const n)  s
evalExpInt (Var x)    s = evalExp (Var x)    s
evalExpInt (VarInc x) s = evalExp (VarInc x) s
evalExpInt (VarDec x) s = evalExp (VarDec x) s
evalExpInt (UMinus e) s = evalExp (UMinus e) s
evalExpInt (Plus  e0 e1) s = let (n0 :!: s') = evalExpInt e0 s 
                                 (n1 :!: s'') = evalExpInt e1 s'
                             in (n0 + n1 :!: s'')
evalExpInt (Minus e0 e1) s = let (n0 :!: s') = evalExpInt e0 s 
                                 (n1 :!: s'') = evalExpInt e1 s'
                             in (n0 - n1 :!: s'')
evalExpInt (Times e0 e1) s = let (n0 :!: s') = evalExpInt e0 s 
                                 (n1 :!: s'') = evalExpInt e1 s'
                             in (n0 * n1 :!: s'')
evalExpInt (Div   e0 e1) s = let (n0 :!: s') = evalExpInt e0 s 
                                 (n1 :!: s'') = evalExpInt e1 s'
                             in (n0 `div` n1 :!: s'')


evalExpBool::Exp Bool -> State -> Pair Bool State
evalExpBool BTrue  s = evalExp BTrue s
evalExpBool BFalse s = evalExp BFalse s
evalExpBool (Not p) s = case evalExpBool p s of
                         (True :!: s')  -> (False :!: s')
                         (False :!: s') -> (True :!: s')
evalExpBool (And p0 p1) s = let (b0 :!: s')  = evalExpBool p1 s
                                (b1 :!: s'') = evalExpBool p0 s'
                            in ((b0 && b1) :!: s'')  
evalExpBool (Or p0 p1)  s = let (b0 :!: s')  = evalExpBool p1 s
                                (b1 :!: s'') = evalExpBool p0 s'
                            in ((b0 || b1) :!: s'')  
evalExpBool (Lt e0 e1)  s = let (n0 :!: s')  = evalExpInt e0 s
                                (n1 :!: s'') = evalExpInt e1 s'
                            in ((n0 < n1) :!: s'')
evalExpBool (Gt e0 e1)  s = let (n0 :!: s')  = evalExpInt e0 s
                                (n1 :!: s'') = evalExpInt e1 s'
                            in ((n0 > n1) :!: s'')
evalExpBool (Eq e0 e1)  s = let (n0 :!: s')  = evalExpInt e0 s
                                (n1 :!: s'') = evalExpInt e1 s'
                            in ((n0 == n1) :!: s'')
evalExpBool (NEq e0 e1) s = let (n0 :!: s')  = evalExpInt e0 s
                                (n1 :!: s'') = evalExpInt e1 s'
                            in ((n0 /= n1) :!: s'')